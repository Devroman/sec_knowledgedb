...
**HTTP метод** - это указание операции над ресурсом, записаное чем угодно, кроме управляющих символов. Однако в подавляющем большинстве используют строго определенные методы для упрощения поддержки и возможности использования сторонних решений(ваш кэп).
...
> Все прочие случаи не предусмотрены спецификацией.
...
Несмотря на все описанные прелести **HTTP/2.0** он еще не пошатнул позиции **HTTP/1.1** потому, что имеет несколько важных недостатков:  
...
> Proxy-pass - это механизм передачи запроса от одного сервера другому.
